defmodule SprintLensWeb.UserAuth do
  @moduledoc """
  Session handling for the browser: signing in, signing out, resolving the
  current scope, and the plugs and `on_mount` hooks that guard routes
  (NFR-201, NFR-206).
  """
  ...
  @remember_me_cookie "_sprint_lens_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      SprintLensWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      SprintLensWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule SprintLensWeb.PageLive do
        use SprintLensWeb, :live_view

        on_mount {SprintLensWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{SprintLensWeb.UserAuth, :require_authenticated}] do
